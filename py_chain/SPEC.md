...
- **信号模式（`signal_mode`，默认 `realtime`，2026-09-05 新增）**：
  - `"realtime"`（当下背驰）：每根 fine（3分钟）收盘评估——低级别**形成中段**（引擎增量
    ...
    **进场MACD柱缩闸**（2026-09-23，`CHAN_CFG.entryMacdShrink` 默认开，参数中心 entry
    模块可关；`BacktestEngine(entry_macd_shrink=)` / `evaluateRealtimeEntries(entryMacdShrink=)`
    形参同名透传）：背驰级别（markRes）最近两根**已收**K线的 MACD 柱状体（`|macd|`）变小
    才出信号——上一根K确认背驰且柱缩，下一根开盘进场；动能仍在放大（末柱 ≥ 前柱）时不发，
    已收柱不足两根保守拦截。闸在背驰候选判定之后、段去重（fired.add）之前：闸未过**不消耗**
    去重键，下一拍自动重评，天然实现"等柱缩才进"。动因：8-20 19:45 wait2Buy 于 20:00 进场、
    20:03 即被支阻位止损（M1 rearm 又两连小止损，磨损约 −53 后错过 20:45 起涨）——进场时
    15m 动能仍在放大。全量 A/B（2026-08-02→09-23 XAUUSD，store+realtime+near10+lead60+lots4）：
    关 42 笔 +303.41（胜率 19.0%、PF 6.45、支阻位止损 21）→ 开 23 笔 **+781.10**（胜率 34.8%、
    PF 6.74、支阻位止损 8）——8 笔盈利单全部保留、剔除 19 笔亏损单；8-20 19:45 磨损簇
    （3 笔 −53.4）整簇消失（20:00/20:15/20:30 三个 15m 边界拍 MACD 柱 −3.8/−5.3/−6.3 逐级放大，
    动能加速期间不进场）。
...
- **震荡判定源改参考周期 + 2买/2卖 DIF 0 轴容差（2026-09-16）**：`compute_plan(range_res=)`（默认 `RANGE_RES="240"`，参数中心 plan.rangeRes；替换语义——更低周期不再做自身 A/B 震荡判定，参考周期 regime 震荡才观望，自身中枢不再封锁）与 `CHAN_CFG.macdZeroTol`（默认 5：2买 `dif > -tol` / 2卖 `dif < +tol`，原严格 `dif>0`/`dif<0`，参数中心 chan 模块可调，0=回退）。动因：2026-06-12 22:45 的 15m wait2Buy 信号（锚点 4177.25）被「15m 中枢内观望」封锁 30 分钟 +「DIF 严格 >0」（DIF 低至 -3.76，反弹拉到 23:15 bar 收盘才转正）再拖 15 分钟，23:30 才以 4225.83 追高成交、保本止损 -44.8——逐拍复现验证（22:42 拍起①够笔 20 根/②背驰候选/③近支阻全过，卡在计划闸与 DIF 闸）。规则全文见 §2.1.2.1 / §2.1.2.4。当日二次收紧：参考周期及以上（≥rangeRes）计划行固定观望只作锚；参考周期笔 <2 → 更低周期观望（不回退自身判定）；rangeRes 必填（枚举无关闭项，空=全部周期观望）。
...
|------|------|--------------------|--------------------|
| `wait2Buy` 等待回调后做 2 买 | 做多 | **创新高** `brokePrevHigh`：最近完成的 up 笔终点突破更早那笔 up 笔终点 | **DIF 未破 0 轴太多** `macdAboveZero`——`dif > -macdZeroTol`（默认容差 5，2026-09-16 前为严格 `dif > 0`；参数中心 chan 模块可调，0=回退严格口径）：上过 0 轴后回调没深破 0 轴，多头动能还在 |
...
- **ATR 分量（2026-09-19）**：三个滑点各有 ATR 系数（默认 0=关闭）——**有效滑点 = 固定值 + 系数 × ATR(14, 背驰周期 markRes)**；ATR 窗口固定 14 内部实现，参数配置页系数内嵌在各滑点行
- 进场信号自带的近支阻位 `nearSr` 若已在正确一侧 → 直接沿用；否则从支阻位列表重选正确侧最近位（进场判定逻辑不变，仅供出场止损参考）
- **没有正确一侧的支阻位 → 兜底止损 = 进场价 ± 有效兜底滑点**（默认 10）
- 结论：**止损位永远不为空**，不存在「不设止损」的仓位
- **进场K线止损下限（2026-09-23，`CHAN_CFG.stopEntryBarFloor` 默认开，参数中心 entry 模块可关；`BacktestEngine(stop_entry_bar_floor=)` 形参）**：止损通常按上述支阻位逻辑，但**至少**推到进场时所在**背驰周期（markRes）K线**极值外侧加滑点处（做多 low−有效止损滑点 / 做空 high+有效止损滑点，与支阻位止损同一份有效滑点；**取更宽者**，支阻位已更远则维持）。进场K线在进场时通常尚未走完（如 20:00 进场、15m K线 20:15 才收），无前视实现：成交时用窗口内已收 fine bar 极值作种子；持仓中该K线每创新低/高，`advance_exit_decision` 用运行极值同步外推（只放松、三模式共用），K线收盘后自然冻结——由构造保证**进场K线内部止损不触发**，此后跌破其最终极值−滑点才止损。保本迁移（beDone）后止损=beStop，下限不再参与。动因：8-20 19:45 信号 20:00 进场、20:03 即被支阻位止损 4462.57 扫掉（M1 rearm 三连小止损 −53.4），而 15m 进场K线最低 4450.745−3=4447.745 全程未破、随后 20:45 起涨。全量 A/B（同 §2.1 闸口径四组）：双关 +303.41 → 仅下限开 +540.01（8-20 那笔单 trade 持有到次日 TP3 全平 **+307.5**，止损冻结 4447.745 与手工复核一致，支阻位止损 21→15）；与柱缩闸叠加（双开=新默认）+777.76——闸已拦下 8-20 簇，下限在其之上的边际贡献约持平（−3.3，stopSr 8→7 / stopBe 9→10），作为独立风险规则保留默认开。

#### 2.1.3.3 保本位 beStop 怎么定

= **进场成交那根 K 线的极值 ± 有效保本滑点**（做空：最高价 + 3；做多：最低价 − 3；含 ATR 分量，同上）。
...
| 出场 | `--slip-stop-atr-k` | 0.0 | 止损滑点 ATR 系数：有效滑点 = 固定值 + 系数×ATR(14,背驰周期)；0=关闭 |
| 出场 | `--slip-fallback` | 10.0 | 兜底止损滑点（无正确侧支阻位时） |
| 出场 | `--slip-fallback-atr-k` | 0.0 | 兜底滑点 ATR 系数（同上口径） |
| 出场 | `--slip-be` | 3.0 | 保本滑点（beStop = 进场K线极值 ± 有效保本滑点） |
| 出场 | `--slip-be-atr-k` | 0.0 | 保本滑点 ATR 系数（同上口径） |
| 出场 | `EXIT_MIN_MERGED` | 5 | 形成段成笔预期门槛（合并后 K 线块数） |
| 进场（当下制） | `REALTIME_MIN_BARS` | 5 | 检测周期形成中回调段的够笔门槛（段长 ≥5 根 K） |
| 进场（当下制） | `entryMacdShrink` | 开 | 进场MACD柱缩闸（2026-09-23，参数中心 entry 模块；`BacktestEngine(entry_macd_shrink=)` 形参）：背驰级别最近两根已收K线柱状体(\|macd\|)变小才出信号，闸未过不消耗段去重键、下一拍自动重评（详见 §2.1 信号模式段） |
| 出场 | `stopEntryBarFloor` | 开 | 进场K线止损下限（2026-09-23，参数中心 entry 模块；`BacktestEngine(stop_entry_bar_floor=)` 形参）：止损至少在进场背驰周期K线极值外侧加滑点处（运行外推→收盘冻结，与支阻位止损取更宽者；详见 §2.1.3.2） |
...
| 2b | **震荡判定源** | 各周期自身 A/B 判定（`isRangeBound` 横盘 / 自身中枢内） | `range_res`（默认 4 小时，参数中心 plan.rangeRes，2026-09-16）**替换语义**：更低周期只看参考周期 regime，自身中枢不再封锁（参考周期自身与 D 固定观望只作锚） |
| 2c | **2买/2卖 DIF 0 轴** | 严格 `dif > 0` / `dif < 0` | 容差 `macdZeroTol`（默认 5，参数中心 chan 模块，2026-09-16）：`dif > -tol` / `dif < +tol`——回调/反弹深破 0 轴才算动能衰竭 |
...
| 事件 | 触发条件 | 动作 |
|------|----------|------|
| 止损 `stopSr` | fine K线**盘中**破坏止损位（`stop_ref_of`：正确侧最近支阻位 ± 有效止损滑点（含 ATR 分量）；**无正确侧位兜底 = 进场价 ± 有效兜底滑点**，止损位永不为 None；**进场K线下限**（§2.1.3.2，默认开）：止损至少在进场背驰周期K线极值外侧加滑点处、该K线运行中同步外推、内部不触发；跳空按开盘价成交） | 全平终局 |
...
- **与 JS 的口径差**：见 §2.1.5.2（逐项差异表），此处不再重复。
...
- 交易计划取最近 60 笔；震荡判定 A（isRangeBound）与 B（未离开中枢且当前价在 ZD~ZG 内）优先于趋势策略。判定源自 `range_res`（2026-09-16 起默认 4 小时，替换语义：更低周期只看参考周期 regime，详见 §2.1.2.1）。
