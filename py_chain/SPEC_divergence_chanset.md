...
3. **规则 3（归属）**：markRes = 下沉停止的那一级；该级末段 vs 段内前一同向段执行 创新高/低 + MACD 背驰 + 近支阻 校验，通过才发信号。
   > **修订（2026-09-11，当下制）**：停止级无候选时沿链向上一级（仍严格 < 检测周期）回退
   > 重评，首个命中级为 markRes（信号带 fallback=True），配套同向持仓终局后重置段去重——
   > 背景/开关/全量回测对比见 `spec/plans/SPEC_divergence_fallback.md`（CHAN_CFG
   > sinkFallback / sinkFallbackRearm，默认开）。确认制（sinkChainConfirm）不受影响。
