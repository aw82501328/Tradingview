...
本技能已移植到 Claude Code（与 Cursor 版共用 `.cursor/skills/mark-entry/scripts/mark_entry.js`，脚本保持单一来源）：

- **斜杠命令**：对话框输入 `/mark-entry` 直接调用；
- **意图触发**：直接说「标记进出场 / 进出场」等，Claude 会自动匹配并加载本技能；
- **加载后**：Claude 按本文档执行——先核对前置条件（TradingView Desktop 9222、图表停在目标品种，且**已依序运行画笔 → 买卖点 → 支阻互换位 → 交易计划**，四份落盘 json 齐全），**必须先向用户询问起始日期 `--from`（见下方「起始日期规则」，未获日期不得运行脚本）**，再用 Bash 运行正文的 node 命令，最后汇报各背驰级别的进场信号。
