...
## 出场规则与同向持仓互斥（出场阶梯重构 2026-09-09）

每个进场信号从进场时刻起按时间顺序模拟出场（`simulatePosition`，纯函数；与 py_chain 引擎同口径）：

| 事件 | 触发条件 | 动作 |
|------|----------|------|
| **止损 stopSr** | 背驰级别K线盘中破坏止损位（short `high>` 位 / long `low<` 位；跳空按开盘价成交） | 全平终局，黄 `↓/↑` |
| **止盈1 保本 breakeven** | 背驰周期**够笔**：进场后首笔有利方向笔（short→down / long→up）完成 | 止损位上移至**保本止损位 beStop**（仅落盘，不画图） |
| **止盈2 平一半 half** | **仅顺势**（计划 direction ∈ {多头多, 空头空}）：检测周期首个有利方向、**合并后 ≥5 根K且有成笔预期**的形成段 | 平一半（下一开盘成交），剩余半仓止损移至 beStop，黄 `↓/↑` |
| **止盈3 全平 close** | 顺势：检测周期**有利方向笔破前高/前低**；逆势（多头空/空头多）：检测周期首个有利方向形成段（合并后 ≥5 根K成笔预期） | 全平终局（下一开盘成交），黄 `↓/↑` |
