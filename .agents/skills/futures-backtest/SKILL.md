...
本技能已移植到 Codex（与 Cursor 版共用 `.cursor/skills/futures-backtest/scripts/ema_cross_backtest.js`）：

- **斜杠命令**：对话框输入 `/futures-backtest` 直接调用；
- **意图触发**：用户要求对 纳指/期货/外汇/币圈 品种做策略回测、评价均线买卖信号、统计胜率/回撤/盈亏比时自动加载；
- **加载后**：Codex 先核对前置条件（TradingView Desktop 9222、图表停在目标品种），必要时先调用「open-tradingview」拉起，再用 Bash 运行 `node .cursor/skills/futures-backtest/scripts/ema_cross_backtest.js <周期> <EMA> <年数>`（参数见下方说明），最后解读统计表（低胜率高盈亏比是趋势策略的正常特征）。
