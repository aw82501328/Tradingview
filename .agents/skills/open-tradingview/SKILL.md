...
本技能已移植到 Codex（与 Cursor 版共用 `.cursor/skills/open-tradingview/scripts/open_tradingview.js`）：
...
- **加载后**：Codex 先检查 9222 端口（已监听则告知用户直接可用、不重复启动），未监听则用 Bash 运行 `node .cursor/skills/open-tradingview/scripts/open_tradingview.js`，并按下方「输出说明」汇报。
