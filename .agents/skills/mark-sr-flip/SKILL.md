...
本技能已移植到 Codex（与 Cursor 版共用 `.cursor/skills/mark-sr-flip/scripts/mark_sr_flip.js`，脚本保持单一来源）：

- **斜杠命令**：对话框输入 `/mark-sr-flip` 直接调用；
- **意图触发**：直接说「标记支阻互换位 / 标记支撑阻力 / 画支撑阻力线 / mark S/R flip zones」等，Codex 会自动匹配并加载本技能；
- **加载后**：Codex 按本文档执行——先核对前置条件（TradingView Desktop 9222、图表停在目标品种、已对该品种画笔生成 `bis_<品种>.json`），**先向用户询问起始日期 `--from`（见下方「起始日期规则」）**，再用 Bash 运行正文的 node 命令，最后汇报支阻位清单与落盘文件。
